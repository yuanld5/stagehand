...
interface ObserveResult {
  ...
  backendNodeId?: number;  // Internal browser node ID
...
<ParamField path="method" type="string" optional>
  Suggested interaction method: `"click"`, `"type"`, `"hover"`, `"select"`.
</ParamField>
...
const [loginButton] = await page.observe("find login button");
if (loginButton) {
...
const iframeElements = await page.observe({
  instruction: "find form fields in embedded content",
  iframes: true
});
```
...
form_fields = await page.observe("locate form input fields")
...
# Working with results
login_buttons = await page.observe("find login button")
if login_buttons:
...
// Element validation
const requiredElements = await page.observe("find login form");
